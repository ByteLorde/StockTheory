...
export class FinancialsTabComponent implements AfterViewInit {
  ...
  @Input() company : CompanyModel = StockMarketService.selectedCompany;
  ...
  loadCashFlow() {
    ...
    for ( let i = 0; i < quarters.length; i++ ) {
      let quarter = quarters[i];
      let values = [];
      values.push(this.formatToNum(quarter.grossProfit));
      values.push(this.formatToNum(quarter.costOfRevenue));
      values.push(this.formatToNum(quarter.operatingRevenue));
      values.push(this.formatToNum(quarter.totalRevenue));
      values.push(this.formatToNum(quarter.operatingIncome));
      values.push(this.formatToNum(quarter.netIncome));
      values.push(this.formatToNum(quarter.researchAndDevelopment));
      values.push(this.formatToNum(quarter.operatingExpense));
      this.tableData[i] = values.slice();
      this.headerData.unshift(quarter.reportDate);
      let data = {
        label: quarters[i].reportDate,
  ...
  formatToNum(value : number) : number {
    if (!value) return 0;
    let formatted = value;
    if (value > 1000000000 || value < -1000000000) {
