...
export class StockMarketService {
  ...
  static request(request : string) {
    return iex.request(request);
  }

  static chooseDate(input: Date) {
    let date : string = input.toISOString().slice(0,10);
    while (date.indexOf("-") > -1) {
      date = date.replace('-', '');
    }


    return this.selectedCompany.getDate(date);
}
